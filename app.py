...
#             # 1. Counselling Line (Upper Row)
#             fig.add_trace(
#                 go.Scatter(x=course_df[week_col], y=course_df[couns_perc], 
#                            name='Counselling %', mode='lines+markers+text',
#                            line=dict(color='#3498DB', width=3),
#                            text=course_df[couns_perc].astype(str)+'%', textposition='top center'),
#                 row=1, col=1
#             )

#             # 2. Retention Line (Lower Row)
#             fig.add_trace(
#                 go.Scatter(x=course_df[week_col], y=course_df[ret_perc], 
#                            name='Retention %', mode='lines+markers+text',
#                            line=dict(color='#E74C3C', width=3),
#                            text=course_df[ret_perc].astype(str)+'%', textposition='top center'),
#                 row=2, col=1
#             )

#             # --- CUSTOM Y-AXIS SCALES ---
#             # Counselling Axis: Gap set to 1 (0, 1, 2, 3, 4, 5...)
#             fig.update_yaxes(title_text="Conv %", dtick=1, gridcolor='#3E4255', row=1, col=1)
            
#             # Retention Axis: Gap set to 10 (0, 10, 20, 30...)
...
# --- DATA LOADING LOGIC (Updated for Google Sheets) ---
...
if source_type == "Google Sheet (Personal)":
    gsheet_url = st.sidebar.text_input("Google Sheet Link dalo:", placeholder="https://docs.google.com/spreadsheets/d/...")
    if gsheet_url:
        df = load_gsheet(gsheet_url)
        if df is not None:
            st.sidebar.success("✅ Sheet Loaded!")
        else:
            st.sidebar.error("❌ Link check karo (Anyone with link hona chahiye)")
...
# --- Processing & Dashboard (Aapka Original Logic) ---
if df is not None:
    df.columns = df.columns.str.strip()
    ...
    st.info("👈 Sidebar mein link dalo ya data folder check karo.")